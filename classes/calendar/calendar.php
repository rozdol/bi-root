	...
	public function standard($name)
	{
		...
		switch ($name)
		{
			...
			case 'holidays':
				...
				// Attach Epiphany
				$holiday = clone $event;
				$this->attach($holiday->condition('month', 1)->condition('day', 6)->title('Epiphany')->output('Epiphany'));

				// Attach Greek Independance
				$holiday = clone $event;
				$this->attach($holiday->condition('month', 3)->condition('day', 25)->title('Greek Independance')->output('Greek Independance'));

				// Attach Cyprus National Day
				$holiday = clone $event;
				...
				// Attach Easter
				$holiday = clone $event;
				$this->attach($holiday->condition('easter_fr', TRUE)->title('Good Friday')->output('Good Friday'));
				...
				// Attach Easter
				$holiday = clone $event;
				$this->attach($holiday->condition('easter_mo', TRUE)->title('Good Friday')->output('Easter Monday'));
				
				// Attach Easter
				$holiday = clone $event;
				$this->attach($holiday->condition('easter_tu', TRUE)->title('Good Friday')->output('Easter Tuesday'));
				
				// Attach Easter
				$holiday = clone $event;
				$this->attach($holiday->condition('easter_gr', TRUE)->title('Green monday')->output('Green monday'));
				
				// Attach Easter
				$holiday = clone $event;
				$this->attach($holiday->condition('easter_pc', TRUE)->title('Pentecost')->output('Pentecost'));
				*/

				// Attach Labor Day
				$holiday = clone $event;
				$this->attach($holiday->condition('month', 5)->condition('day', 1)->title('Labor Day')->output('Labor Day'));

				// Attach Assumption day
				$holiday = clone $event;
				$this->attach($holiday->condition('month', 8)->condition('day', 15)->title('Assumption day')->output('Assumption day'));

				// Attach Cyprus Independance Day
				$holiday = clone $event;
				$this->attach($holiday->condition('month', 10)->condition('day', 1)->title('Cyprus Independance Day')->output('Cyprus Independance Day'));
				
				// Attach Greek National Day
				$holiday = clone $event;
				$this->attach($holiday->condition('month', 10)->condition('day', 28)->title('Greek National Day')->output('Greek National Day'));
