...
if ((int)$id <= 0) $id = $this->html->readRQn('id');
$noduplicate = $this->html->readRQn('noduplicate');
$duplicate = ($noduplicate > 0) ? 0 : 1;
$redirect = $this->html->readRQ('redirect');

if ($_POST['backtoedit'] > 0) {
	unset($_POST['backtodetails']);
	$where = "?act=edit&what=$GLOBALS[what]&duplicate=0&backtoedit=1&id=$_POST[id]";
}
...
if ($GLOBALS['info_message'] != '') {
	$stop = 1;
	$body .= $this->html->refreshpage($where, 60, $this->html->message($GLOBALS['info_message'], "INFO", 'alert alert-info'));
}
if ($GLOBALS['error_message'] != '') {
	//echo $this->html->pre_display($where,"where"); exit;
	$stop = 1;
	$body .= $this->html->refreshpage($where . "&noduplicate=$noduplicate&error_message=" . $GLOBALS['error_message'], 3, $this->html->message("$GLOBALS[error_message]", "ERROR", 'alert alert-error'));
}
