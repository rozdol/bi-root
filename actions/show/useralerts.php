...
if ($what == 'useralerts') {
    ...
    $tmp = $this->html->readRQn('sent');
    if ($tmp > 0) {
        $sql = "$sql and a.fromuserid=$uid";
    }
    $tmp = $this->html->readRQn('received');
    if ($tmp > 0) {
        $sql = "$sql and a.userid=$uid";
    }
    $tmp = $this->html->readRQn('showall');
    if ($tmp > 0) {
        $sql = "";
        $showall = 1;
    }
    ...
    $tmp = $this->html->readRQn('unread');
    if ($tmp > 0) {
        $sql = "$sql and wasread='0'";
    }
    $tmp = $this->html->readRQn('wasread');
    if ($tmp > 0) {
    ...
    while ($row = pg_fetch_array($cur)) {
        ...
        $totals[1] = (float) $totals[1] + (float) $row['share'];
    }
